...
2. 執行上方的 Mac 安裝命令，或在專案根目錄執行 `bash mac/install.sh`。按提示輸入兩端 IP，腳本會安裝 tinyproxy，監聽所有介面的 `8888` 埠，並放行本機、機器人，以及 `192.168.10.0/24` 網段內的其他機器。監聽 `0.0.0.0` 是為了在未插上拓展塢、Mac 還沒有網線 IP 時也能成功啟動。
